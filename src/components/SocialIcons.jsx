...
const svgProps = { width: ICON_SIZE, height: ICON_SIZE, viewBox: '0 0 24 24', fill: ICON_COLOR, 'aria-hidden': true }
...
export function SocialIcons({ social, className = '' }) {
  ...
  const wrap = (url, label, Icon) => {
    ...
    return (
      <span className="social-icon social-icon--disabled" style={{ ...iconWrapStyle, opacity: 0.45 }} aria-label={`${label} (not set)`}>
        <Icon />
      </span>
    )
  }
