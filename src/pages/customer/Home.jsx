...
export function CustomerHome() {
  ...
  const nearby = MOCK_VENDORS.filter(v => v.city === 'San Jose')
  ...
  return (
    <div className="page animate-fadeIn">
      ...
      {notifBanner && (
        <div style={{ margin: '12px 16px 0', padding: '12px 14px', background: 'rgba(249,156,76,0.08)', border: '1px solid rgba(249,156,76,0.2)', borderRadius: 'var(--radius)', display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <button onClick={() => { localStorage.setItem('elrincon_notif_dismissed', 'true'); setNotifBanner(false) }} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', padding: 2, flexShrink: 0 }}>
            <X size={14} />
          </button>
        </div>
      )}
      ...
      <div style={{ padding: '16px 16px 0' }}>
        ...
        <h2 style={{ fontSize: 22, fontWeight: 800, lineHeight: 1.2, marginBottom: 16 }}>
          ...
          <span style={{ fontSize: 18, fontWeight: 500, color: 'var(--text-secondary)' }}>Auténticas Carnitas, Entregadas.</span>
        </h2>
        ...
        <div onClick={() => navigate('/vendors')} style={{
          ...
          <span style={{ color: 'var(--text-muted)', fontSize: 15 }}>Find your nearest El Rincón location...</span>
        </div>
      </div>
      ...
      <div style={{ margin: '0 16px 24px', position: 'relative', height: 140, borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(15,10,8,0.88) 0%, rgba(15,10,8,0.3) 100%)', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 20px' }}>
          ...
          <div style={{ fontSize: 18, fontWeight: 800, lineHeight: 1.2, marginBottom: 8 }}>Carnitas Party Trays,<br />Ready to Feast</div>
      ...
      <div className="scroll-x" style={{ paddingBottom: 8, marginBottom: 24 }}>
        {MEAT_CATEGORIES.map(cat => (
          <button key={cat.id} onClick={() => setSelectedCategory(selectedCategory === cat.id ? null : cat.id)} style={{
            ...
            background: selectedCategory === cat.id ? 'rgba(249,156,76,0.15)' : 'var(--bg-card)',
      ...
      {!selectedCategory && (
        <>
          ...
          <div className="section-header">
            <span className="section-title">🔥 Featured Locations</span>
            <button onClick={() => navigate('/vendors')} className="btn btn-ghost btn-sm text-muted" style={{ fontSize: 13, padding: '4px 0' }}>All 19</button>
          </div>
          <div className="scroll-x" style={{ marginBottom: 24 }}>
          ...
          {/* Nearby */}
          <div className="section-header">
            <span className="section-title">📍 San Jose Locations</span>
          </div>
      ...
      {selectedCategory && (
        <>
          <div className="section-header">
            <span className="section-title">Locations Near You</span>
          ...
          <div style={{ padding: '0 16px' }}>
            {categoryVendors.length === 0 ? (
              <div className="empty-state">
                ...
                <div className="empty-desc">No El Rincón locations in your area right now</div>
              </div>
